...
STEM = "FigS4_sample_level_robustness_revised_v2_minorfix"
...
def build_panel_b(ax: plt.Axes, reg_mat: pd.DataFrame, fig: plt.Figure) -> None:
    ...
    ax.tick_params(length=0)
    cbar = fig.colorbar(im, ax=ax, fraction=0.045, pad=0.025)
    cbar.set_label("Regulon ΔAUC", fontsize=7.0)
    cbar.ax.tick_params(labelsize=6.1, width=0.5, length=2.2)
    panel_label(ax, "B")
...
def build_panel_c(ax: plt.Axes, pseudo: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("Pseudobulk regulon ΔAUC")
...
def build_panel_d(ax: plt.Axes, retained: pd.DataFrame, ranks: pd.DataFrame) -> None:
    ...
    for i, tf in enumerate(mat.index):
        for j, col in enumerate(mat.columns):
            if mat.iloc[i, j] == 1:
                ...
                ax.text(j, i, "NR", ha="center", va="center", fontsize=5.7, color="#666666", fontweight="bold")
    for spine in ax.spines.values():
        spine.set_visible(False)
    legend_handles = [
        Patch(facecolor="#485F66", edgecolor="#485F66", label="Retained"),
        Patch(facecolor="#E9E9E9", edgecolor="#BDBDBD", label="NR = not retained"),
        Line2D([0], [0], color="none", label="R# = retained rank under each threshold setting"),
    ]
    ax.legend(
    ...
    ax.text(
        0.0,
        -0.42,
        "Each column shows retention after varying the indicated threshold; direction agreement was required and the positive-fraction threshold was fixed at 0.",
        transform=ax.transAxes,
        ha="left",
        va="top",
        fontsize=5.9,
...
Minorfix changes:
- Panel B colorbar was changed from Regulon dAUC to Regulon ΔAUC.
- Panel C y-axis was changed from Pseudobulk regulon dAUC to Pseudobulk regulon ΔAUC.
- Panel C label offsets were adjusted for NFE2L2, HMGA1, and BHLHE40 without changing point positions.
- Panel D not-retained cell labels were changed from not to NR.
- Panel D legend now states NR = not retained and R# = retained rank under each threshold setting.
- Panel D threshold rule was updated to: each column shows retention after varying the indicated threshold; direction agreement was required and the positive-fraction threshold was fixed at 0.
...
- Pseudobulk regulon difference was read from 03_pseudobulk_regulon_table.csv and represents lesion-minus-internal-control regulon ΔAUC at the sample level.
...
- NR indicates not retained for that threshold setting.
- R# indicates retained rank under each threshold setting.
...
- It uses sample-excluded analyses because the discovery cohort has four samples but only two donors.
- It is not a separate verification analysis.
...
def print_summary(outputs: list[Path], companions: list[Path]) -> None:
    ...
    print("FigS4 revised_v2 minorfix output path:")
    for path in outputs:
        print(f"- {rel(path)}")
    print("Input files:")
    for path in INPUT_FILES:
        print(f"- {rel(path)}")
    print("dAUC changed to ΔAUC: yes")
    print("Panel D not-retained cell label changed from not to NR: yes")
    print("Panel D R# legend updated: yes")
    print("Panel D threshold rule updated: yes")
    ...
    print("Original data unchanged: yes")
    print("Minorfix notes generated: " + ("yes" if all(path.exists() for path in companions) else "no"))
