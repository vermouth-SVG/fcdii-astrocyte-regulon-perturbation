...
STEM = "FigS3_celloracle_perturbation_metrics_revised_v2"
...
FIGURE_TEXT_ITEMS = [
    ...
    "Round1 CellOracle perturbation metric heatmap",
...
def build_panel_a(ax: plt.Axes, metrics: pd.DataFrame, fig: plt.Figure) -> None:
    ...
    ax.set_title("Round1 CellOracle perturbation metric heatmap", pad=7)
...
def draw_vector_field(
    ...
    sampled = sample_vector_rows(df, 240, seed)
    if mode == "simulated":
    ...
    ax.quiver(
        ...
        scale=1.25,
        color=arrow_color,
        alpha=0.66 if mode == "simulated" else 0.52,
        width=0.0036,
        headwidth=3.2,
        headlength=4.0,
        headaxislength=3.5,
...
def build_tf_block(
    ...
    sub = parent_spec.subgridspec(2, 2, height_ratios=[0.18, 1.0], wspace=0.055, hspace=0.045)
...
def build_figure(metrics: pd.DataFrame, vectors: dict[str, pd.DataFrame]) -> plt.Figure:
    ...
    gs = fig.add_gridspec(
        4,
        2,
        height_ratios=[1.12, 1.42, 1.42, 0.22],
        hspace=0.50,
        wspace=0.33,
        left=0.065,
        right=0.985,
...
def caption_text() -> str:
    return (
        ...
        "(A) Column-scaled heatmap of CellOracle round1 perturbation metrics for NFE2L2, THRB, "
        "BHLHE40, and SOX2. Metrics include mean shift, Approx. Recovery Index, net shift, "
...
def notes_text(metrics: pd.DataFrame) -> str:
    ...
    return f"""FigS3_celloracle_perturbation_metrics_revised_v2 notes
...
def revision_log_text() -> str:
    return """# FigS3 CellOracle Perturbation Metrics Revised V2 Revision Log

- Rebuilt only Supplementary Figure S3 outputs under manuscript_output/figures_supplementary_revised_v2.
...
- Did not modify main Figure 1-5 or Supplementary FigS1, FigS2, FigS4, FigS5, FigS6, or FigS7.
"""
...
def terminology_audit_df() -> pd.DataFrame:
    rows = [
        ...
        ("experimental validation", "absent", "required absent"),
        ("internal_control", "absent", "replaced by internal-control in figure-facing labels"),
...
def write_companion_files(metrics: pd.DataFrame) -> list[Path]:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    caption_path = OUT_DIR / f"{STEM}_caption.md"
    notes_path = OUT_DIR / f"{STEM}_notes.txt"
    log_path = OUT_DIR / f"{STEM}_revision_log.md"
    audit_path = OUT_DIR / f"{STEM}_terminology_audit.csv"
    caption_path.write_text(caption_text(), encoding="utf-8")
    notes_path.write_text(notes_text(metrics), encoding="utf-8")
    log_path.write_text(revision_log_text(), encoding="utf-8")
    terminology_audit_df().to_csv(audit_path, index=False)
    return [notes_path, caption_path, log_path, audit_path]
...
def print_summary(outputs: list[Path], companions: list[Path], residuals: dict[str, bool]) -> None:
    print("FigS3 revised_v2 output path:")
    for path in outputs:
        print(f"- {rel(path)}")
    print("Input files:")
    for path in INPUT_FILES:
        print(f"- {rel(path)}")
    print("Recovery Index wording standardized to Approx. Recovery Index / Approx. RI: yes")
    print("Randomized control retained and standardized as negative-control assessment: yes")
    print("Method note added: yes")
    flagged = [term for term, present in residuals.items() if present]
    if flagged:
        print("Residual forbidden terms in figure/caption/log text: " + "; ".join(flagged))
    else:
        print("Residual forbidden terms in figure/caption/log text: none")
    generated = all(path.exists() for path in companions)
    print("Caption, notes, revision log, terminology audit generated: " + ("yes" if generated else "no"))
    print("Panels requiring manual review: none identified; please visually inspect vector density and label placement before submission.")
