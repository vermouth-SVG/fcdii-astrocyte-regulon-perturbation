...
STEM = "FigS6_functional_enrichment_convergence_revised_v2_minorfix"
...
def _sentence_case_token(token: str, is_first: bool) -> str:
    if token in PRESERVE_TOKENS:
        return token
    if "-" in token:
        return "-".join(_sentence_case_token(part, is_first and idx == 0) for idx, part in enumerate(token.split("-")))
    if token.upper() in PRESERVE_TOKENS:
        return token.upper()
    if re.fullmatch(r"[A-Z]{2,}\d*", token):
        return token
    lowered = token.lower()
    if is_first and lowered:
        return lowered[0].upper() + lowered[1:]
    return lowered


def sentence_case_term(term: object) -> str:
    text = str(term)
    ...
    for piece in pieces:
        if piece.isspace() or piece == "":
            out.append(piece)
            continue
        out.append(_sentence_case_token(piece, first_word))
        first_word = False
    return "".join(out)
...
def build_enrichment_panel(ax: plt.Axes, enrich: pd.DataFrame, tf: str, label: str) -> None:
    ...
    ax.set_yticks(y)
    ax.set_yticklabels([wrap_term(sentence_case_term(t), 40) for t in sub["term_short"]])
...
def build_figure() -> plt.Figure:
    ...
    fig.legend(handles=legend_handles, loc="lower center", ncol=2, frameon=False, bbox_to_anchor=(0.52, 0.030))
...
Boundary notes:
- Minorfix display updates: Panels A-D show GO/KEGG terms in sentence case without changing source terms or ordering.
- Minorfix display updates: Panel E uses THRB (compact homeostatic-supportive), and Panel F note placement was lowered to avoid x-axis label overlap.
- Original enrichment, FDR, overlap gene count, overlap ratio, and regulon target values were not changed.
...
def scan_residual_terms() -> dict[str, bool]:
    scanned = "\n".join([*FIGURE_TEXT_ITEMS, notes_text()])
    forbidden = [
        "validated pathway",
        "confirmed mechanism",
        "therapeutic target",
...
def print_summary(outputs: list[Path], companions: list[Path]) -> None:
    ...
    print("FigS6 revised_v2 minorfix output path:")
    for path in outputs:
        print(f"- {rel(path)}")
    print("Input files:")
    for path in INPUT_FILES:
        print(f"- {rel(path)}")
    print("A-D term display changed to sentence case: yes")
    print("THRB label changed to compact homeostatic-supportive: yes")
    print("Panel F note position optimized: yes")
    print("Contextual KEGG muting retained: yes")
    ...
    print("Original enrichment and overlap data unchanged: yes")
