...
STEM = "Fig4_main_v31_robustness_support"
...
def load_panel_a_data(warnings: list[str]) -> tuple[pd.DataFrame, dict[str, str]]:
    ...
    used_cols = {
        ...
        "Raw median rank": f"{median_rank_col or 'not available'} (not displayed in Panel A)",
    }
    return df, used_cols
...
def load_panel_b_data(warnings: list[str]) -> tuple[pd.DataFrame, pd.DataFrame, list[str], dict[str, str], bool]:
    ...
    order_cols = ["Full"] + [f"−{str(label).replace('.', '')}" for label in label_map[label_col].tolist()]
    rename_map = {
        str(label): f"−{str(label).replace('.', '')}"
...
def plot_panel_a(ax: plt.Axes, data: pd.DataFrame) -> None:
    ...
    ax.axis("off")
    ax.set_title("Internal robustness scorecard", pad=8)
    ...
    ax.text(class_x, header_y, "Class", fontsize=6.3, fontweight="bold", ha="left", va="bottom")
...
def plot_panel_b(ax: plt.Axes, raw: pd.DataFrame, scaled: pd.DataFrame, order_cols: list[str], fig: plt.Figure) -> None:
    ...
    ax.set_title("LOSO axis direction stability", pad=8)
    ...
    cbar.set_label("row-scaled signed effect")
...
def plot_panel_c(ax: plt.Axes, g140: pd.DataFrame) -> None:
    ...
    for yi, (_, row) in zip(y, g140.iterrows()):
        ax.text(
            ...
            yi,
            f"{float(row['support_score_plot']):.2f}",
            color=TEXT_GREY,
            fontsize=6.5,
            fontweight="normal",
            va="center",
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(g140["tf"].tolist())
    for tick in ax.get_yticklabels():
        tf = tick.get_text()
        tick.set_color(TF_COLORS.get(tf, TEXT_GREY))
...
def write_notes(
    ...
    Revision scope:
    - Figure 4 v3.1 is a layout and graphical-language revision of v3.
    - No data values or upstream analyses were changed.
    ...
    - Panel A v3.1 deletes the raw median-rank small text and keeps only normalized rank stability in the main visual.
    - Panel A values:
    {textwrap.indent(compact_table(panel_a[["tf", "expr_loso", "regulon_loso", "shortlist_retention", "rank_stability", "robustness_class"]]).strip(), "    ")}
    ...
    - Panel B color indicates row-scaled signed effect; overlaid numbers are raw effect values.
    - Raw effect table:
    ...
    - Panel C v3.1 is a ranked lollipop using Support score on the x-axis; point size uses positive fraction when available.
    - Panel C values:
    ...
    - Panel D support class text was shortened in v3.1: NFE2L2 = disease-side support; THRB = control-like partial; SOX2 = partial support; BHLHE40 = attenuated.
    - Panel D values:
    ...
    Figure 4 v3.1 uses existing robustness and external supportive-analysis outputs only; no upstream analysis was rerun.
