...
INDEX_PATH = ROOT / "manuscript_output" / "SUPPLEMENTARY_PACKAGE_INDEX_SUBMISSION.md"
        ...
        [
            {
                ...
                "Cells": f"{int(float(merge.get('h5ad_cells', '2322'))):,}",
                "Object": "astrocyte pilot / AUCell-matched",
                "Comparison": "lesion vs internal_control",
                "Manuscript_status": "main discovery",
            },
            {
                "Dataset": "GSE140393",
                "Role": "single-group supportive analysis",
                "Cells": f"{int(float(build140.get('cells', '11399'))):,}",
                "Object": "lesion-only supportive object",
                "Comparison": "lesion only",
                "Manuscript_status": "supportive only",
            },
            {
                "Dataset": "GSE190452",
                "Role": "cross-syndrome supportive analysis",
                "Cells": f"{int(float(build190.get('cells', '69968'))):,}",
                "Object": "cross-syndrome supportive object",
            ...
            },
            {
                ...
                "Cells": "-",
                "Object": "planned external route",
                "Comparison": "not available",
                "Manuscript_status": "not included",
            },
            {
                ...
                "Manuscript_status": "archived only",
            },
        ]
    )
    out = MAIN_TABLE_DIR / "Table1_cohort_and_analytical_objects_submission.csv"
...
def build_table_supplementary() -> list[Path]:
    ...
    s1 = read_csv("analysis_outputs/group_compare/regulon_group_statistics.csv").sort_values(["fdr_bh", "abs_mean_diff"], ascending=[True, False]).copy()
    s1["Higher_axis"] = np.where(as_num(s1["mean_lesion"]) > as_num(s1["mean_internal_control"]), "lesion", "internal_control")
    s1["dAUC_lesion_minus_control"] = as_num(s1["mean_lesion"]) - as_num(s1["mean_internal_control"])
    s1 = s1[["regulon", "Higher_axis", "mean_lesion", "mean_internal_control", "dAUC_lesion_minus_control", "fdr_bh"]].rename(
        columns={
            "regulon": "Regulon",
            "mean_lesion": "Mean_AUC_lesion",
            "mean_internal_control": "Mean_AUC_control",
            "fdr_bh": "FDR",
        }
    )
    for col in ["Mean_AUC_lesion", "Mean_AUC_control", "dAUC_lesion_minus_control"]:
        s1[col] = s1[col].map(lambda x: round_or_blank(x, 3))
    s1["FDR"] = s1["FDR"].map(sci_or_blank)
    p1 = TABLE_DIR / "TableS1_full_differential_regulons_submission.csv"
    ...
    s2["Axis"] = s2["regulon_effect_group"].map({"lesion": "lesion-associated", "internal_control": "internal_control-associated"})
    s2 = s2[
        [
            "tf",
            "regulon",
    ...
    for col in ["Expr_log2FC", "Regulon_dAUC", "Pos_frac_high_group"]:
        s2[col] = s2[col].map(lambda x: round_or_blank(x, 3))
    s2["Expr_FDR"] = s2["Expr_FDR"].map(sci_or_blank)
    s2["Regulon_FDR"] = s2["Regulon_FDR"].map(sci_or_blank)
    p2 = TABLE_DIR / "TableS2_candidate_tf_shortlist_extended_submission.csv"
    s2.to_csv(p2, index=False)
    outputs.append(p2)

    s3 = read_csv("celloracle_run/ko_round1/round1_ko_master_table.csv").sort_values("mean_shift_length", ascending=False).copy()
    s3 = s3[
        [
            "tf",
            "mean_shift_length",
        ...
        columns={
            ...
            "lesion_mean_shift_length": "Lesion_shift",
            "internal_control_mean_shift_length": "Control_shift",
            "direction_agreement_factor": "Direction_agreement",
            "group_selectivity": "Group_selectivity",
        }
    )
    for col in ["Mean_shift", "Net_shift", "Recovery_index", "Lesion_shift", "Control_shift", "Direction_agreement", "Group_selectivity"]:
        s3[col] = s3[col].map(lambda x: round_or_blank(x, 3))
    p3 = TABLE_DIR / "TableS3_celloracle_round1_full_metrics_submission.csv"
    s3.to_csv(p3, index=False)
    outputs.append(p3)

    s4 = read_csv("robustness_validation/05_robustness_validation_integrated_table.csv").copy()
    stability = read_csv("robustness_validation/04_shortlist_priority_stability.csv")[["tf", "retention_frequency", "median_rank_when_retained", "best_rank", "worst_rank"]]
    s4 = s4.merge(stability, on="tf", how="left")
    s4 = s4[
        [
            "tf",
            "full_data_expr_direction",
            "full_data_regulon_direction",
            "loo_expr_consistency_rate",
            "loo_regulon_consistency_rate",
            "pseudobulk_direction",
            "pseudobulk_effect_size",
            "pseudobulk_regulon_direction",
            "shortlist_retention_frequency",
            "retention_frequency",
            "median_rank_when_retained",
            "best_rank",
            "worst_rank",
        ...
        columns={
            "tf": "TF",
            "full_data_expr_direction": "Discovery_expr_direction",
            "full_data_regulon_direction": "Discovery_regulon_direction",
            "loo_expr_consistency_rate": "LOSO_expr_rate",
            "loo_regulon_consistency_rate": "LOSO_regulon_rate",
            "pseudobulk_direction": "Pseudobulk_expr_direction",
            "pseudobulk_effect_size": "Pseudobulk_expr_effect",
            "pseudobulk_regulon_direction": "Pseudobulk_regulon_direction",
            "shortlist_retention_frequency": "Integrated_retention",
            "retention_frequency": "Grid_retention",
            "median_rank_when_retained": "Median_rank",
            "best_rank": "Best_rank",
            "worst_rank": "Worst_rank",
            "final_internal_robustness_level": "Robustness",
            "recommended_priority_tier": "Priority_tier",
        }
    )
    for col in ["LOSO_expr_rate", "LOSO_regulon_rate", "Pseudobulk_expr_effect", "Integrated_retention", "Grid_retention", "Median_rank"]:
        s4[col] = s4[col].map(lambda x: round_or_blank(x, 3))
    p4 = TABLE_DIR / "TableS4_sample_level_robustness_full_submission.csv"
    s4.to_csv(p4, index=False)
    outputs.append(p4)

    enrich = pd.concat(
        [
            read_csv("functional_interpretation/03_go_enrichment_all.csv").assign(Source="GO"),
            read_csv("functional_interpretation/03_kegg_enrichment_all.csv").assign(Source="KEGG"),
        ],
        ignore_index=True,
        sort=False,
    )
    enrich["fdr_bh"] = as_num(enrich["fdr_bh"])
    top_terms = enrich.sort_values(["tf", "fdr_bh"]).groupby(["tf", "Source"]).head(5)
    conv = read_csv("functional_interpretation/04_program_convergence_table.csv")
    s5 = top_terms.merge(
        conv[["tf", "overlap_count", "overlap_ratio", "program_theme_1", "program_theme_2"]],
        on="tf",
        how="left",
    )
    s5 = s5[
        ...
        columns={
            ...
            "candidate_tier": "Tier",
            "term": "Term",
            "overlap_count_x": "Term_overlap_count",
            "overlap_ratio_x": "Term_overlap_ratio",
            "fdr_bh": "FDR",
            "overlap_count_y": "Program_overlap_count",
            "overlap_ratio_y": "Program_overlap_ratio",
            "program_theme_1": "Theme_1",
            "program_theme_2": "Theme_2",
        }
    )
    for col in ["Term_overlap_ratio", "Program_overlap_ratio"]:
        s5[col] = s5[col].map(lambda x: round_or_blank(x, 3))
    s5["FDR"] = s5["FDR"].map(sci_or_blank)
    p5 = TABLE_DIR / "TableS5_functional_interpretation_program_convergence_full_submission.csv"
    s5.to_csv(p5, index=False)
    outputs.append(p5)

    g140 = read_csv("external_validation/single_group_support/external_support_ranking.csv").copy()
    g140["dataset"] = "GSE140393"
    g140["analysis"] = "single-group supportive"
    g140 = g140[
        [
            "dataset",
            "analysis",
    ...
    g190 = read_csv("external_validation_round2/external_round2_support_ranking.csv").copy()
    g190["dataset"] = "GSE190452"
    g190["analysis"] = "cross-syndrome supportive"
    g190 = g190[
        [
            "dataset",
            "analysis",
    ...
    s6 = pd.concat([g140, g190], ignore_index=True, sort=False).rename(
        columns={
            "tf": "TF",
            "expected_higher_group_main": "Expected_axis",
            "lesion_presence_score": "Lesion_presence_score",
            "direction_consistency_score": "Direction_score",
            "group_log2fc_lesion_vs_internal_control": "Group_log2FC",
            "cluster_support_fraction": "Cluster_support_fraction",
            "support_score": "Support_score",
            "support_rank": "Support_rank",
        }
    )
    for col in ["Lesion_presence_score", "Direction_score", "Group_log2FC", "Cluster_support_fraction", "Support_score"]:
        if col in s6.columns:
            s6[col] = s6[col].map(lambda x: round_or_blank(x, 3))
    p6 = TABLE_DIR / "TableS6_supportive_external_analysis_full_submission.csv"
    s6.to_csv(p6, index=False)
    outputs.append(p6)

    s7 = read_csv("drug_repositioning/16_paper_ready_table_after_bbb_v21.csv").copy()
    s7["mechanism_theme"] = s7["mechanism_theme"].map(clean_mechanism_theme)
    s7["Boundary"] = "exploratory_only"
    s7 = s7[
        [
            "compound",
            "axis",
        ...
        columns={
            "compound": "Compound",
            "axis": "Axis",
            "mechanism_theme": "Mechanism_theme",
            "prebbb_layer_v21": "PreBBB_layer",
            "after_bbb_final_layer": "AfterBBB_layer",
            "BBB_layer": "BBB_layer",
            "final_score_after_bbb": "Final_score_after_BBB",
            "display_role": "Display_role",
            "Boundary": "Interpretation_boundary",
        }
    )
    s7["Final_score_after_BBB"] = s7["Final_score_after_BBB"].map(lambda x: round_or_blank(x, 3))
    p7 = TABLE_DIR / "TableS7_drug_repositioning_v21_after_bbb_full_submission.csv"
...
def build_figS1() -> list[Path]:
    ...
    ax_a.scatter(sample_counts["n_cells"], y, s=52, c=[GROUP_COLORS.get(g, "#777777") for g in sample_counts["group"]], edgecolor="white", linewidth=0.8, zorder=3)
    ax_a.set_yticks(y)
    ax_a.set_yticklabels(sample_counts["clinical_sample_label"])
    ax_a.set_xlabel("Cells in final analysis object")
    ax_a.set_title("Final 4-sample pilot composition", pad=6)
    ...
    return save_figure(fig, "FigS1_discovery_object_qc_and_input_overview_submission")
...
def build_figS2() -> list[Path]:
    ...
    return save_figure(fig, "FigS2_full_differential_regulon_landscape_submission")
...
def build_figS3() -> list[Path]:
    ...
    return save_figure(fig, "FigS3_extended_celloracle_prioritization_submission")
...
def build_figS4() -> list[Path]:
    ...
    for ax, thr, label in zip([ax_d1, ax_d2, ax_d3], [0.01, 0.05, 0.10], ["D", None, None]):
        ...
        for spine in ax.spines.values():
            spine.set_visible(False)
    return save_figure(fig, "FigS4_sample_level_robustness_full_submission")
...
def build_figS5() -> list[Path]:
    ...
    return save_figure(fig, "FigS5_supportive_external_evidence_full_submission")
...
def build_figS6() -> list[Path]:
    ...
    return save_figure(fig, "FigS6_functional_interpretation_and_program_convergence_full_submission")
...
def build_figS7() -> list[Path]:
    ...
    count_mat = layer_counts.reindex(columns=["NFE2L2", "THRB", "BHLHE40", "SOX2"]).fillna(0)
    im0 = ax_a.imshow(count_mat.values, aspect="auto", cmap="OrRd", vmin=0, vmax=max(1.0, float(count_mat.values.max())))
    ax_a.set_xticks(np.arange(count_mat.shape[1]))
    ax_a.set_xticklabels(count_mat.columns, rotation=25, ha="right")
    ax_a.set_yticks(np.arange(count_mat.shape[0]))
    ax_a.set_yticklabels([short_layer_name(x) for x in count_mat.index])
    for i in range(count_mat.shape[0]):
        for j in range(count_mat.shape[1]):
            ax_a.text(j, i, str(int(count_mat.iloc[i, j])), ha="center", va="center", fontsize=6.1, color="white" if count_mat.iloc[i, j] >= 2 else "#222222")
    for spine in ax_a.spines.values():
        spine.set_visible(False)
    ax_a.set_title("After-BBB layer count matrix", pad=6)
    fig.colorbar(im0, ax=ax_a, fraction=0.05, pad=0.03, label="count")
    panel_label(ax_a, "A")
    ...
    top_labels = reviewed.sort_values("final_score_after_bbb", ascending=False).head(6)
    ...
    return save_figure(fig, "FigS7_drug_repositioning_v21_after_bbb_summary_submission")
...
def write_index(table1: Path, figures: dict[str, list[Path]], supp_tables: list[Path]) -> None:
    ...
    - `{table1.relative_to(ROOT).as_posix()}`: Compact Table 1 for the manuscript main text.
