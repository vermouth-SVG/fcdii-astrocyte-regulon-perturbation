...
STEM = "Fig2_main_v3_regulon_landscape"
TITLE = (
...
CONTROL_COLOR = "#0B5D6B"
LESION_COLOR = "#8E1F2F"
BHLHE40_COLOR = "#7B5EA7"
SOX2_COLOR = "#8C8C8C"
LIGHT_GREY = "#D8D8D8"
MID_GREY = "#777777"
GRID_GREY = "#ECECEC"
...
def add_sample_annotation_strip(ax: plt.Axes, sample_meta: pd.DataFrame, donor_colors: dict[str, str]) -> None:
    strip = ax.inset_axes([0.0, 1.012, 1.0, 0.115])
    ...
    strip.text(-0.62, 1.5, "group", fontsize=5.8, ha="right", va="center", clip_on=False)
    strip.text(-0.62, 0.5, "donor", fontsize=5.8, ha="right", va="center", clip_on=False)
    strip.set_xticks([])
    strip.set_yticks([])
...
def plot_panel_a(
    ...
    if 0 < n_control < heat.shape[0]:
        ax.axhline(n_control - 0.5, color="#FFFFFF", linewidth=2.1)
    ...
    cbar = fig.colorbar(im, ax=ax, fraction=0.036, pad=0.026)
    cbar.set_label("row z-score")
    cbar.ax.tick_params(labelsize=5.8, width=0.5, length=2.2)
    ax.set_title("Sample-level regulon activity", pad=24)
    add_panel_label(ax, "A", x=-0.17, y=1.16)
...
def plot_panel_b(ax: plt.Axes, stats: pd.DataFrame) -> None:
    ...
    ax.text(0.02, 0.07, "internal-control-associated", transform=ax.transAxes, color=CONTROL_COLOR, fontsize=6.0, ha="left")
    ax.text(0.98, 0.07, "lesion-associated", transform=ax.transAxes, color=LESION_COLOR, fontsize=6.0, ha="right")
    ...
    for reg in PANEL_B_LABELS:
        ...
        ax.scatter([x], [y], s=42, color="white", edgecolor=TF_COLORS.get(tf, "#333333"), linewidth=1.0, zorder=4)
...
def plot_axis_slope(
    ...
    for donor in donor_order:
        ...
        ax.text(1.03, ys[-1], donor, fontsize=5.8, va="center", color=MID_GREY)
...
def plot_panel_c(fig: plt.Figure, spec, axis_table: pd.DataFrame) -> None:
    sub = spec.subgridspec(1, 2, wspace=0.20)
    ...
    add_panel_label(ax_c1, "C", x=-0.25)
...
def plot_panel_d(ax: plt.Axes, scorecard: pd.DataFrame, cmap: LinearSegmentedColormap) -> None:
    ...
    n = len(rows)
    ax.set_xlim(-0.42, 6.25)
    ax.set_ylim(-0.75, n + 0.75)
    ...
    for x, label in headers:
        ax.text(x, n + 0.18, label, fontsize=6.4, fontweight="bold", ha="center", va="bottom")
    ...
    dauc_norm = TwoSlopeNorm(vmin=-max_abs_dauc, vcenter=0, vmax=max_abs_dauc)
    expr_norm = TwoSlopeNorm(vmin=-max_abs_expr, vcenter=0, vmax=max_abs_expr)
    max_fdr = max(1.0, float(np.nanmax(rows["neg_log10_regulon_fdr"])))
    max_pos = max(0.01, float(np.nanmax(rows["positive_fraction"])))
    ...
    for i, row in rows.iterrows():
        ...
        if tf in {"NFE2L2", "THRB"}:
            ...
            ax.add_patch(Rectangle((-0.38, y - 0.36), 6.46, 0.72, facecolor=face, edgecolor="none", zorder=0))
        elif tf == "BHLHE40":
            ax.add_patch(Rectangle((-0.38, y - 0.36), 6.46, 0.72, facecolor="#F3EFF8", edgecolor="none", zorder=0))
        elif tf == "SOX2":
            ax.add_patch(Rectangle((-0.38, y - 0.36), 6.46, 0.72, facecolor="#F5F5F5", edgecolor="none", zorder=0))

        ax.hlines(y - 0.47, -0.38, 6.08, color="#EEEEEE", linewidth=0.55, zorder=0)
        ax.text(
            x_tf,
            y,
            tf + "(+)",
        ...
        ax.text(x_axis, y, str(row["axis_display"]), fontsize=6.1, ha="center", va="center", color=TF_COLORS.get(tf, MID_GREY), alpha=alpha)
        ...
        ax.add_patch(
            Rectangle(
                ...
                0.46,
                0.46,
                facecolor=scorecard_value_color(dauc, dauc_norm, cmap),
                edgecolor="white",
                linewidth=0.6,
                alpha=alpha,
            )
        ...
        dot_size = 20 + 75 * math.sqrt(max(0.0, neg_fdr) / max_fdr)
        ax.scatter([x_fdr], [y], s=dot_size, color=TF_COLORS.get(tf, MID_GREY), edgecolor="white", linewidth=0.6, alpha=0.86 * alpha, zorder=3)
        ...
        ax.add_patch(
            Rectangle(
                ...
                0.46,
                0.46,
                facecolor=scorecard_value_color(expr, expr_norm, cmap),
                edgecolor="white",
                linewidth=0.6,
                alpha=alpha,
            )
        ...
        pos = float(row["positive_fraction"])
        ax.add_patch(Rectangle((x_pos - 0.31, y - 0.07), 0.62, 0.14, facecolor="#E7E7E7", edgecolor="none", zorder=1))
        ax.add_patch(
            Rectangle(
                (x_pos - 0.31, y - 0.07),
                0.62 * max(0.0, pos) / max_pos,
                0.14,
                facecolor=TF_COLORS.get(tf, MID_GREY),
                edgecolor="none",
                alpha=0.82 * alpha,
                zorder=2,
            )
        )
        ax.text(x_pri, y, str(row["priority_class"]), fontsize=6.1, ha="center", va="center", color="#222222", alpha=alpha)

    ax.text(2.96, -0.55, "control-associated color left; lesion-associated color right", ha="center", va="top", fontsize=5.6, color=MID_GREY)
    ax.set_title("Integrated TF prioritization matrix", loc="left", pad=7)
    add_panel_label(ax, "D", x=-0.03, y=1.04)
...
def write_notes(
    ...
    Script:
    - scripts/manuscript_figures/build_fig2_main_v3_regulon_landscape.py
    ...
    Figure 2 v3 uses existing pySCENIC/AUCell outputs only; no pySCENIC or CellOracle rerun was performed.
    """
...
def main() -> None:
    ...
    gs = fig.add_gridspec(3, 2, width_ratios=[1.08, 1.0], height_ratios=[1.0, 0.72, 1.18], wspace=0.10, hspace=0.18)
    ax_a = fig.add_subplot(gs[:, 0])
    ax_b = fig.add_subplot(gs[0, 1])
    ax_d = fig.add_subplot(gs[2, 1])

    fig.suptitle(TITLE, x=0.012, y=0.995, ha="left", va="top", fontsize=10.0, fontweight="bold")
